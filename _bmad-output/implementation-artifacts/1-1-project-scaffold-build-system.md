...
- [ ] Create the baseline project structure (AC: 1, 2)
  - [ ] Add `include/`, `src/`, `data/`, `build/`, and `bin/`
...
- [ ] Create the initial Makefile (AC: 1, 2, 3)
  ...
  - [ ] Use `-Wall -Wextra` at minimum
  - [ ] Add `make clean`
  - [ ] Add `make format`
- [ ] Align the scaffold with architecture rules (AC: 1)
  - [ ] Keep `src/main.c` limited to entry-point scaffolding only
...
- [ ] Verify the story outcome locally (AC: 1, 2, 3)
  ...
  - [ ] Run `make clean`
  - [ ] Run `make format`
  - [ ] Confirm the executable path and generated directory behavior
...
- Story intentionally scoped to scaffold only; feature work belongs to later stories
